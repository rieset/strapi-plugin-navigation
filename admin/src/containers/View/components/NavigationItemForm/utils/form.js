    ...
    return yup.object({
      ...
      externalPath: yup.string()
        .when('type', {
          ...
          then: yup.string()
            .required(translatedErrors.required)
            .matches(/(#.*)|(https?:\/\/(?:www\.|(?!www))[a-zA-Z0-9][a-zA-Z0-9-]+[a-zA-Z0-9]\.[^\s]{2,}|www\.[a-zA-Z0-9][a-zA-Z0-9-]+[a-zA-Z0-9]\.[^\s]{2,}|https?:\/\/(?:www\.|(?!www))[a-zA-Z0-9]+\.[^\s]{2,}|www\.[a-zA-Z0-9]+\.[^\s]{2,}|mailto:[a-zA-Z0-9]+@[a-zA-Z0-9]+\.[^\s]{2,})/, { 
